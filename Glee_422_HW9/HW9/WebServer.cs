...
namespace CS422
{
    ...
    {
        private static readonly HashSet<WebService> _services = new HashSet<WebService> {};
        ...
        */
        private static void ThreadWork()
        {
            while (true)
            {
                ...
                {
                    WebService service = GetService(request);
         ...
         */
        private static WebService GetService(WebRequest request)
        {
            foreach (var service in _services)
            {
                if (request.RequestURI.StartsWith(service.ServiceURI, StringComparison.CurrentCulture))
                {
                    return service;
                }
            }
