	...
	class MainClass
	{
		...
		private static void Q9()
		{
			...
			for (int threadNum = 0; threadNum < threads.Length; threadNum++)
			{
				...
				{
                    for (int i = 0; i < 100000; i++) { Interlocked.Increment(ref s_sharedValue); } //{ s_sharedValue++; }
				});
		...
		// Find a way to initialize stringValue to a non-null string that causes the "doc.ToString()" 
		// call on the last line to throw an exception. You must not alter any code other than the 
		// hard-coded value for stringValue, which again, cannot be null. Explain the string you chose 
