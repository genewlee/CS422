...
namespace CS422
{
    public static class WebServer
    {
        ...
        //public statis bool Start(Int32 port, Int32 nThreads)
        //{
        //}
        ...
        private static WebRequest BuildRequest(TcpClient client)
        {
            ...
            if (!ReadRequestFromClient(client, ref buf))
            {
                client.GetStream().Dispose();
                client.Close();
                return null;
            }
            WebRequest ws = new WebRequest();
            ws.networkStream = client.GetStream();

            // Get http method
            ws.HTTPMethod = Encoding.ASCII.GetString(buf).Substring(0).Split(' ')[0];
            ...
            Regex ver = new Regex(@"\d+\.\d+");
            string http = regex.Match(stringOfBuf).ToString();
            ws.HTTPVersion = ver.Match(http).ToString();

            // Get request target
            ws.RequestTarget = Encoding.ASCII.GetString(buf).Substring(4).Split(' ')[0];

            // Make dictionary of headers
            ConcurrentDictionary<string, string> Headers = new ConcurrentDictionary<string, string>();
            string[] HttpRequest = Encoding.ASCII.GetString(buf).Substring(0).Split('\n');
            foreach (var line in HttpRequest)
            {
                var kvp = line.Substring(0).Split(':');
                var key = kvp[0];
                var value = kvp[1];
                Headers[key] = value;
            }
